...
def graph_metric(
    ...
    # (The metric name is already communicated via the title.)
...
# %% Let's look at the ones that 007 is still getting wrong
import importlib
...
failures = (
    df.filter(pl.col("model") == "email-agent-007")
    .filter(pl.col("metric_answer_correct") == 0)
    .filter(pl.col("split") == "val")
    .filter(pl.col("step") == 389)
)
print(len(failures))

# failures.drop("messages", "logs")

for i, row in enumerate(failures.rows(named=True)):
    email_address = row["metadata_email_inbox"]
    query = row["messages"][1]["content"]
    scenario = None

    for s in scenarios:
        if s.question == query and s.inbox_address == email_address:
            scenario = s
            break
    if scenario is None:
